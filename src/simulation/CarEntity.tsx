...
import {useJitRef} from '@/helpers/hooks';
import {useCommandQueue} from '@/state/useCommandStore';
import {useAppStore} from '@/state/useAppStore';
import {CanvasRes} from '@/simulation/lib/commandlib';
...
export const WheelJoint = ({
  ...
  useFrame(() => {
    if (joint.current && controls.current && controls.current.cmds.getDrive(side) != wheelVel.current) {
      joint.current.configureMotorVelocity(controls.current.cmds.getDrive(side) * WHEEL_VEL, WHEEL_FAC);
...
export const PokeyJoint = ({body, pokey, bodyAnchor, pokeyAnchor, tranAxis, ctrl}: PokeyProps) => {
  ...
  useFrame(() => {
    if (joint.current && ctrl.current && ctrl.current.cmds.getProbe('Y') != pokeyPos.current) {
      joint.current.configureMotorPosition(ctrl.current.cmds.getProbe('Y') / 30, 1000, 20);
...
export function CarEntity({
  ...
  const floatyBoxesRef = useJitRef(() => sensorPositions.map(() => createRef<THREE.Mesh>()));
  const floatBoxesColorRef = useJitRef(() => sensorPositions.map(() => createRef<THREE.MeshPhysicalMaterial>()));
  const wheelRefs = useJitRef(() => wheelPositions.map(() => createRef<RapierRigidBody>()));
  ...
  useFrame(() => {
    ...
    if (!floatBoxesColorRef.current || !bodyRef.current || !parentBox) return;
  ...
  return (
    <group position={position} rotation={rotation}>
      <RigidBody
        ...
        canSleep={false}
        friction={0}
        collisionGroups={interactionGroups(BODY_COL_GROUP, [MAIN_COL_GROUP, BODY_COL_GROUP])}
      >
        <Box ref={floatyBoxesRef.current[0]} position={sensorPositions[0]}>
          <group position={[30, 10, 0]} rotation={[0, Math.PI / 2, 0]}>
            <PerspectiveCamera position={[0, 0, 10]} />
          </group>
          <Html>
            {canvasRef.current && autoRef.current && (
              <Readout canvasRef={canvasRef.current} controlRef={autoRef.current} bodyRef={bodyRef} />
            )}
            <div style={labelStyle} className='select-none'>
              0
            </div>
            {!auto && (
              <div style={goBtn} className='select-none' onClick={handleStart}>
                Start
              </div>
            )}
          </Html>
          <meshStandardMaterial color={'red'} ref={floatBoxesColorRef.current[0]} />
          {floatyBoxesRef.current.slice(1).map((ref, index) => (
            <Box ref={ref} key={index + 1} position={sensorPositions[index + 1]}>
              <Html>
                <div style={labelStyle} className='select-none'>
                  {index + 1}
                </div>
              </Html>
              <meshStandardMaterial color={'red'} metalness={1} ref={floatBoxesColorRef.current[index + 1]} />
            </Box>
          ))}
        </Box>
        ...
        <Cylinder rotation={[Math.PI / 2, 0, 0]} position={[-3, -0.3, -3.3]} args={[WHEEL, WHEEL, WHEEL, 16]} />
        <BallCollider friction={-0.1} args={[WHEEL]} position={[-3, -0.3, 3.3]} />
        <Cylinder rotation={[Math.PI / 2, 0, 0]} position={[-3, -0.3, 3.3]} args={[WHEEL, WHEEL, WHEEL, 16]} />
        <BallCollider friction={-0.1} args={[WHEEL]} position={[-3, -0.3, -3.3]} />
      </RigidBody>
      {wheelPositions.map((wheelPosition, index) => (
        <RigidBody
          ...
          friction={2}
          collisionGroups={interactionGroups(WHEEL_COL_GROUP, [MAIN_COL_GROUP])}
        >
          <Cylinder rotation={[Math.PI / 2, 0, 0]} args={[WHEEL, WHEEL, WHEEL, 16]}>
      ...
      {/* pokey boy */}
      <RigidBody position={pokeyPosition} type='dynamic' ref={pokeyRef} friction={1}>
        {/* plate */}
...
function Readout_({canvasRef, controlRef, bodyRef}: ReadoutProps) {
  ...
  useEffect(() => {
    ref.current = setInterval(() => {
      if (controlRef && bodyRef.current) {
        ...
        let [xs, ys, zs] = [x, y, z].map(v => v.toFixed(1).padEnd(5, ' '));
        const loc = `x:${xs} y:${ys} z:${zs}`;
        const [LW, RW] = controlRef.cmds._drive.map(x => String(x).padEnd(4, ' '));
        let LR = `LW:${LW} RW:${RW}`;
        const [T, L, B, R] = canvasRef.luminance.map(x => String(x).padEnd(4, ' '));
        const LUM = `T:${T} L:${L}\nB:${B} R:${R}`;
  ...
  }, [bodyRef, canvasRef.luminance, controlRef]);
