import {Image as Image3D, Box, Cylinder} from '@react-three/drei';
...
const Floor = () => {
  return (
    <RigidBody type='fixed' colliders='cuboid'>
      <Box position={[0, -12, 0]} scale={[200, 10, 200]} rotation={[0, 0, 0]}>
  ...
  return (
    <group position={[-43.24, -5, -11.94]}>
      ...
      <RigidBody type='fixed' friction={0}>
        <Box scale={[2, 2.95, 5.74]} position={[1.1, -0.01, 0]} material={YELLOW_STD} receiveShadow />
        {/* FLOOR */}
        <Box scale={[4.2, 0.5, 5.74]} position={[0, -1.75, 0]} material={YELLOW_STD} receiveShadow />
        {/* SLIDE */}
        <Box
          scale={[2.5, 0.5, 6]}
          position={[-0.67, 0.1, 0]}
        ...
        {/* FRONT BOX */}
        <Box scale={[2.7, 3.5, 4.1]} position={[6.39, -0.75, -1.46]} material={YELLOW_STD} />
        {/* RESTBOX */}
...
const FuelUnit = (props: RigidBodyProps) => (
  <RigidBody gravityScale={0.8} {...props} scale={0.8} friction={0.4} mass={2} colliders='cuboid'>
    <Cylinder args={[2 / 3, 2 / 3, 1, 10]} castShadow material={HOTPINK_PHYS} />
  </RigidBody>
);
...
const PushyBoy = forwardRef<RapierRigidBody>(function PushyBoy(_, ref) {
  return (
    <RigidBody enabledTranslations={[false, true, false]} lockRotations={true} ref={ref} position={[8.24, -1, -1.46]}>
...
const UnitStack = ({pushyRef}: {pushyRef: RefObject<RapierRigidBody>}) => {
  ...
  useFrame(() => {
    ...
    if (pusherRef.current && pushyRef && pushyRef.current) {
      ...
      const mapX = mapLinear(sourceZ, sourceStart, sourceEnd, targetStart, targetEnd);
      trans.x = mapX;
  ...
  useLayoutEffect(() => {
    setTimeout(() => {
      if (FS_PUSHER_REF.target.start === -1 && pusherRef.current && pushyRef.current) {
    ...
    <>
      <group position={[0.5, 2, -2]}>
        ...
        <RigidBody type='fixed'>
          <Box scale={[0.2, 5, 0.2]} position={[HOLDER_RAD, 3.1, 0]} material={YELLOW_STD} />
          <Box scale={[0.2, 5, 0.2]} position={[-HOLDER_RAD, 3.1, 0]} material={YELLOW_STD} />
          <Box scale={[0.2, 5, 0.2]} position={[0, 2.1, HOLDER_RAD - 0.1]} material={YELLOW_STD} />
          <Box scale={[0.2, 5, 0.2]} position={[0, 2.1, -HOLDER_RAD + 0.1]} material={YELLOW_STD} />
          {/* SLIDE GUIDE*/}
          <Box scale={[0.2, 5, 2]} position={[-2, 1, 0]} material={YELLOW_STD} />
        </RigidBody>
        {/* pusher */}
        <RigidBody ref={pusherRef} type='fixed'>
          <Box scale={[1.2, 2 / 3, 1]} position={[1.3, -0.1, 0]} material={GRAY_STD} />
        </RigidBody>
        {Array(stored)
          .fill(0)
          .map((_, i) => (
            <FuelUnit position={[0, i * 1.01, 0]} key={i + 1} rotation={[Math.PI / 2, Math.PI / 2, 0]} />
          ))}
      </group>
    </>
  );
