    ...
    return function __intersection() {
      ...
      const [dark, light] =
        lock === 'R'
          ...
          ? [[], [top, bot, rgt, lft]]
          : [[], []];
      const res = dark.every(x => x < 0.4) && light.every(x => x > 0.8);
      if (res) {
        next();
        return true;
      }
...
export const stop: CommandMaker = trigger => {
