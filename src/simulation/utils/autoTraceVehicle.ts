import {
  ...
  stop,
} from '@/simulation/lib/commandlib';
...
export default class AutoTraceVehicle {
  ...
  setCommandQueue(queue: (CommandBarMakerUnit | null)[]) {
    this.state.cmds.queue = queue.filter(x => x !== null) as CommandBarMakerUnit[];
  }
  assignNextCommandBar() {
    ...
    let trigId = 0;
    const parseCommandBarMakerUnit = (cbmu: CommandBarMakerUnit): [CommandMaker[], Trigger] => {
