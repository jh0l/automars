...
import {CanvasRes, Luminance} from '@/simulation/lib/commandlib';

type FloatyBoxesType = React.MutableRefObject<
  RefObject<THREE.Mesh<THREE.BufferGeometry, THREE.Material | THREE.Material[]>>[]
>;
  ...
  const getColorAtIntersection = () => {
    ...
    if (canvasRef.current.canvas === null) {
      ...
      canvasRef.current.maps = {
        x: (x: number) => Math.floor(map(x, -scale.x / 2, scale.x / 2, 0, width)),
        y: (y: number) => Math.floor(map(y, -scale.y / 2, scale.y / 2, 0, height)),
      };
    } else {
