...
type StateArr = [ControlState, React.Dispatch<React.SetStateAction<ControlState>>];
export function useControls(canvasRef: RefObject<CanvasRes>) {
  const stateArr = useState({auto: false});
  const [state, setState] = stateArr;
  const ref = useJitRef<AutoTraceVehicle>(() => new AutoTraceVehicle(canvasRef.current!));
...
function useKeyboard(ctrl: RefObject<AutoTraceVehicle>, stateArr: StateArr) {
  ...
  const keys = useJitRef<KeyMapType<boolean>>(() =>
    Object.values(keyMap).reduce((a, b) => ({...a, [b]: false}), {} as any),
  );
  useKeyPresses((key, b) => {
    if (key in keyMap && keyMap[key] in keys.current && ctrl.current) {
      keys.current[keyMap[key]] = b;
      const {forward, backward, left, right, brake, auto, sample, reset, paused, probe_up, probe_down} = keys.current;
